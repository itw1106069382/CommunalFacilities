  ...
  <div class="layui-body">
    ...
    <div style="padding: 0px;" class="layui-anim" data-anim="layui-anim-up" >
    	  <button type="button" class="layui-btn layui-btn-primary layui-btn-sm">
		    <a class="layui-icon"  href="${pageContext.request.contextPath }/anomaly/findAnomaly">&#xe654;</a>
		  </button>
 ...
 var laypage = layui.laypage;
  ...
  function del(data){
		 layer.confirm('确定要删除选中的记录吗？', {icon:3,title:'删除确认'}, function(index){
			$.post('<c:url value="/anomaly/delAnomaly"/>',{id:data.id},function(res){
					//重载表格
					table.reload('test');
			})
				layer.close(index);
  ...
  function edit(data){
	  ...
	  //alert(rows)
		...
		window.top.location.href = '<c:url value="/anomaly/getAnomalyById"/>' + '?id=' + rows.data[0].id+'&s='+1;
  }
