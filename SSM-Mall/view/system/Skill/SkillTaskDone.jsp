      ...
      <ul class="layui-nav layui-nav-tree"  lay-filter="test">
        <li class="layui-nav-item layui-nav-itemed">
          ...
          <dd><a href="${pageContext.request.contextPath }/view/system/Skill/SkillIndex.jsp">任务列表</a></dd>
           <dd><a href="${pageContext.request.contextPath }/view/system/Skill/SkillTaskDone.jsp" style="color:rgb(6,218,255)">已完成</a></dd>
...
<script type="text/html" id="barDemo">
		<a class="layui-btn layui-btn-xs" lay-event="edit">恢复任务</a>
</script>
<script>
//JavaScript代码区域
...
layui.use('table', function(){
  ...
  table.render({
    elem: '#test'
    ,url:'<c:url value="/anomaly/data?s=3"/>'
    ...
    ,cols: [[
      ...
      ,{field:'area',title:'地点'}
      ,{field:'operate',width:90,title:'恢复任务',toolbar: '#barDemo'}
    ]]
...
//编辑
  function edit(data){
	  layer.confirm('确定要恢复选中的记录吗？', {icon:3,title:'恢复确认'}, function(index){
			$.post('<c:url value="/anomaly/regainAnomaly"/>',{id:data.id},function(res){
					table.reload('test');
			})
				layer.close(index);
	});
